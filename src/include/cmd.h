...
#define PRINT_ERROR(UTILITY, FORMAT, ...)               \
do                                                      \
{                                                       \
    fprintf(stderr, "%s: %s: " FORMAT "\n",             \
            SOURCE_NAME, UTILITY, ##__VA_ARGS__);       \
} while(0)

#define READONLY_ASSIGN_ERROR(utility, name, type)      \
    PRINT_ERROR(utility, "cannot set `%s`: readonly %s",\
                name, type)
    
#define INVALID_JOB_ERROR(utility, jobstr)              \
    PRINT_ERROR(utility, "invalid job id: %s", jobstr);

#define MISSING_ARG_ERROR(utility, arg)                 \
    PRINT_ERROR(utility, "missing argument: %s", arg);
    
#define MISSING_ARGS_ERROR(utility)                     \
    PRINT_ERROR(utility, "missing arguments");

#define INSUFFICIENT_MEMORY_ERROR(utility, what)        \
    PRINT_ERROR(utility, "insufficient memory for %s", what);

#define OPTION_UNKNOWN_ERROR(utility, opt)              \
    PRINT_ERROR(utility, "unknown option: -%c", opt);
    
#define OPTION_UNKNOWN_ERROR2(utility, sign, opt)       \
    PRINT_ERROR(utility, "unknown option: %c%c", sign, opt);

#define OPTION_UNKNOWN_STR_ERROR(utility, opt)          \
    PRINT_ERROR(utility, "unknown option: %s", opt);
    
#define OPTION_REQUIRES_ARG_ERROR(utility, opt)         \
    PRINT_ERROR(utility, "option requires an argument -- %c", opt);
...
/* helper macros to test different job flags */
#define FOREGROUND_JOB(j)               flag_set((j)->flags, JOB_FLAG_FORGROUND)
#define NOTIFIED_JOB(j)                 flag_set((j)->flags, JOB_FLAG_NOTIFIED)
#define DISOWNED_JOB(j)                 flag_set((j)->flags, JOB_FLAG_DISOWNED)
#define CONTROLLED_JOB(j)               flag_set((j)->flags, JOB_FLAG_JOB_CONTROL)
#define NOT_RUNNING(status)             (WIFEXITED((status)) || \
                                         WIFSIGNALED((status)) || \
                                         WIFSTOPPED((status)))
#define RUNNING(status)                 (((status) == 0) || !NOT_RUNNING(status))
#define STOPPED(status)                 (((status) != 0) && WIFSTOPPED((status)))
...
#define FLAG_WORDEXP_ALL                (FLAG_PATHNAME_EXPAND | \
                                         FLAG_REMOVE_QUOTES |   \
                                         FLAG_FIELD_SPLITTING)
