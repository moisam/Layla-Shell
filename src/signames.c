...
/*
 * POSIX says that when we fork a child process, it should
 * inherit the signal actions we got from our parent, except
 * for those modified by a trap condition. as we only ignore
 * seven signals, we will need to reset those seven (if they
 * are not trapped).
 * 
 * NOTE: instead of the above, we will use the reset_nonignored_traps(),
 *       defined in trap.c
 */
/*
void reset_signals()
{
    ...
    for( ; i < 7; i++)
    {
        sigaction(core_signals[i], &signal_handlers[core_signals[i]], NULL);
    }
}
*/
...
{
    if(signum <= 0 || signum >= total_signames) return;
    sigaction(signum, &signal_handlers[signum], NULL);
}

struct sigaction *get_sigaction(int signum)
{
    if(signum <= 0 || signum >= total_signames) return NULL;
